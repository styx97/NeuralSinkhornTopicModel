...
def run_ntsm(
    ...
    with session as sess:
        ...
        for epoch in range(num_epochs):
            ...
            for batch in tqdm(range(nb_batches)):
                ...
                if sparse.isspmatrix(X):
                    X = X.toarray()
                #print(type(X), type(word_embeddings), type(input_rec_loss_weight))
                batch_start_time = time.time()
                _, rec_loss_batch, sh_rec_loss_batch, joint_loss_batch = \
                    sess.run([trainer, rec_loss, sh_loss, joint_loss],
                             feed_dict={doc_word_ph: X, word_embeddings_ph: word_embeddings, rec_loss_weight: input_rec_loss_weight})
        ...
        with open(topics_path, 'a') as vis_file:
...
if __name__ == '__main__':
    ...
    parser.add("--sh_iterations", default=50, type=int)
    parser.add("--input_rec_loss_weight", default=0.07, type=float)
    ...
    #create a logfile : specific to nstm code 
    logfile_path = output_dir  / "log.txt"
    open(logfile_path, 'w').close()  #TODO (seems hacky, replace later)
    ...
    run_ntsm(
        ...
        input_rec_loss_weight=args.input_rec_loss_weight, 
